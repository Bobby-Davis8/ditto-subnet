...
func TestHandleGenerateRequiresSupportedVersion(t *testing.T) {
	for _, path := range []string{
		"/generate?seed=42&run_size=small&bench_version=" + strconv.Itoa(protocol.NewestBenchVersion()+1),
		...
		if !strings.Contains(rr.Body.String(), "supported: "+protocol.SupportedBenchVersionsText()) {
