...
func BuildFixtureForVersion(masterSeed int64, benchVersion int, c protocol.ToolCase) Fixture {
	if benchVersion < protocol.BenchVersionV13 {
		return BuildFixture(masterSeed, c)
	}
	f := Fixture{seed: caseSeed(masterSeed, c.ID), version: benchVersion, category: c.Category}
