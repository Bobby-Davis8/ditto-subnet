...
func TestBuildFixtureForVersionIsFrozenBelowV13(t *testing.T) {
	...
	for _, version := range []int{protocol.BenchVersionV2, protocol.BenchVersionV8, protocol.BenchVersionV12} {
		...
		versioned := BuildFixtureForVersion(42, version, c)
...
func TestV13DecoysNotConfiguredUnlessExpected(t *testing.T) {
	...
	web := webCase("cweb")
	webFixture := BuildFixtureForVersion(seed, protocol.BenchVersionV13, web)
	...
	decoyFixture := BuildFixtureForVersion(seed, protocol.BenchVersionV13, decoyCase)
...
func TestV13SettersValidateAgainstInventoryWithoutEchoingCanonical(t *testing.T) {
	...
	f := BuildFixtureForVersion(seed, protocol.BenchVersionV13, c)
	s := NewServer()
	s.Register(c.ID, f)
	ts := httptest.NewServer(s)
	defer ts.Close()
...
func TestV13CoinedFixturesCarryTheNeedleOnlyOnTheBearer(t *testing.T) {
	...
	for seed := int64(1); seed <= 40; seed++ {
		for family, tool := range families {
			...
			web := BuildFixtureForVersion(seed, protocol.BenchVersionV13, webCase("cweb"))
	...
	plain := BuildFixtureForVersion(3, protocol.BenchVersionV13, protocol.ToolCase{ID: "cplain", Category: "world_business_workflow", ExpectedTools: []protocol.ToolSpec{{Name: "list_workflows"}, {Name: "create_workflow"}}})
	if plain.has {
