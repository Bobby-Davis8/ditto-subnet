...
| 13 (in progress) | `2027-05-01` | Public-corpora vocabulary and `/seed` label hygiene (#1825, #1827): the shared world draws places, occupations, organisations, fonts, and colours from frozen SHA-pinned public tables; every session id is opaque and every timestamp comes from a seeded business-hours timeline. The envelope (`profilesV13`) starts equal to v10 and the known vector is pinned only after the sibling v13 issues land. |
