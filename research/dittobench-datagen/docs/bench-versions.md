...
| 13 (pre-activation) | `2027-05-01` | Tool bench: per-seed tool catalog (paraphrased descriptions, `enum` schemas, coined decoy tools), discovery-grounded appearance inventories, coined list/discover fixtures, and the retirement of the `set_model`/`set_font` families. Runtime execution is available; Platform activation remains separate. |
