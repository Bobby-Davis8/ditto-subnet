...
| 13 (pre-activation) | `2027-05-01` | Typed-semantic contract (#1518): label-insufficiency, unregenerable surface, causal model dependence, provenance over containment, bounded money share, grader-only claim sets. Plumbing shipped (version, 250-case envelope, grader-only protocol types, v12-copy surface pass); the twin/pair post-pass and the shadow cost factor ship behind observe/shadow switches; families, grader, and gates land behind `bench_version >= 13`. Advertised by the runtime (`supported_bench_versions`); Platform activation remains a separate owner decision after shadow calibration. |
