...
type World struct {
	Seed int64
	// BenchVersion is the contract the world was generated under. Generate
	// (the v8..v12 path) records BenchVersionV8; GenerateForVersion records the
	// requested version so version-gated levers (v13 public corpora, opaque
	// session ids, jittered timestamps) can be audited from the value itself.
	BenchVersion   int
	UserName       string
	UserCompany    string
	People         []Person
...
// It is the frozen v8..v12 world: every contract from v8 through v12 shares
// these bytes, so it delegates to GenerateForVersion at BenchVersionV8 and no
// version-gated lever below can fire.
func Generate(seed int64, scale int) World {
	return GenerateForVersion(seed, scale, protocol.BenchVersionV8)
}

// GenerateForVersion returns the deterministic world for an explicit contract.
// Versions v8 through v12 are byte-identical to Generate. From v13 (#1825,
// #1827) the world draws its vocabulary from the frozen public corpora in
// internal/publicdata instead of the hand lists below, emits every session id
// as an OpaqueCaseID, and stamps pairs from a seeded business-hours timeline
// instead of a 137-hour stride. The rng draw order for the frozen path is
// untouched: every v13 lever either replaces a draw one-for-one or consumes an
// independent seed stream.
func GenerateForVersion(seed int64, scale int, benchVersion int) World {
	if scale < 1 {
		scale = 1
	}
	if scale > 3 {
