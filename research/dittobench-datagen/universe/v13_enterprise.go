...
func GenerateV13Enterprise(seed int64, teams, edits int) (V13EnterpriseWorld, error) {
	...
	pick := func(domain string, n int) int {
		h := sha256.Sum256([]byte(fmt.Sprintf("enterprise-world-v1/%d/%s", seed, domain)))
	...
	for i := 0; i < teams; i++ {
		...
		// Valid repeated removal/reinstatement exercises non-fixed set histories.
		member := fmt.Sprintf("person-%04d-%d", i, pick(fmt.Sprintf("member/%d", i), 3))
		for j := 0; j < edits; j++ {
			op := "remove"
			if j%2 == 1 {
				op = "add"
