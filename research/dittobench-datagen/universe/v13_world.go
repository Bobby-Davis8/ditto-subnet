...
// GenerateForVersion builds the deterministic world for an explicit benchmark
// contract. Versions below 13 return exactly Generate(seed, scale); v13 appends
// the absence probes and records the contract so staged-correction planning is
// available.
func GenerateForVersion(seed int64, scale, benchVersion int) World {
	w := Generate(seed, scale)
	if benchVersion < protocol.BenchVersionV13 {
		return w
	}
	w.Version = benchVersion
	probes := buildV13Probes(seed, &w)
	w.Probes = &probes
...
func (w World) StagedCorrectionWaves(a V13Allocation, nWaves int) map[string]int {
	out := map[string]int{}
	if w.Version < protocol.BenchVersionV13 {
		return out
	}
	if nWaves < 1 {
		nWaves = 1
