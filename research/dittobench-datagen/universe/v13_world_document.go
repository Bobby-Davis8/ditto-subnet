...
func (w World) v13OrdinaryDocuments() ([]v13WorldDocument, error) {
	...
	for _, p := range w.People {
		...
		add(p.ToolNotePairID, "disposable_receipt", "The narrator has finished reconciling stale contact addresses for the person identified by nickname, after the event named by event_context. The completed action is reconciling stale addresses, not finishing the event or the person. This note is a receipt for that completed contact maintenance. Only this receipt may be deleted once reviewed; canonical contact evidence must not be deleted.", map[string]string{"nickname": p.Nickname, "event_context": p.Context})
