...
type QuestionPlan struct {
	...
	Constraints     []string
	Operations      []string
	// Unanswerable (bench_version 13) marks an absence-proof plan: the records in
	// RequiredPairIDs are what an honest harness searches, the oracle deliberately
	// resolves to nothing, and the correct case outcome is a grounded decline.
	// validatePlan proves the absence instead of proving an answer.
	Unanswerable bool
	// Family (bench_version 13) names the absence-proof family of an unanswerable
	// plan and of its answerable decision twin (see V13AbsenceFamilies). Empty for
	// every ordinary plan.
	Family string

	oracleKind  string
	oracleIndex int
	// asOfAnchor is the same-turn point-in-time anchor of a v13 as-of plan: the
	// instant the question asks the chain state for. Zero for every other plan.
...
func (w World) questionCandidates() []QuestionPlan {
	...
	for i, p := range w.Projects {
		...
		out = append(out, w.projectOutstandingPlan(i))
	...
	for i, trip := range w.Trips {
		...
		out = append(out, w.tripChangedLegCurrentPlan(i))
