...
func FactProfileDigest(profile Profile) (string, error) {
	...
	return universe.V13FactRenderDigest([]any{"fact-renderer-v14", universe.V13FactGenerationRevision, profile, factAuthorPrompt, factCheckPrompt, factPlanSchema(), factVerdictSchema(), factDocumentAuthorPrompt, factDocumentCheckPrompt, factDocumentSchema(1), factDocumentCompactPrompt, factDocumentLayoutPrompt, factDocumentLayoutSchema(), "long-record-opening-evidence-closing-blank-line-assembly", "document-single-record-author-calls-full-document-check-1-through-16", "document-max-two-structural-attempts-per-record", "value-free-length-interior-and-sorted-missing-token-feedback", "author-bindings-withheld", "checker-concrete-assertions", "exact-model-provider-identity", "three-record-token-plan", "max-two-author-structural-attempts", "no-semantic-or-transport-retry", 0.8, 0.0})
}
