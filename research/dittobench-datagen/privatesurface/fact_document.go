...
Do not strengthen a fact while making it sound natural. A nickname establishes an alias, not that everyone uses it; use "also known as" or another equally scoped phrasing, never "everyone calls". Likewise do not add universal frequency, popularity, exclusivity, certainty, or permanence unless the assertion explicitly supplies that scope. Preserve explicitly supplied scope, including exclusive assignments, exactly.
