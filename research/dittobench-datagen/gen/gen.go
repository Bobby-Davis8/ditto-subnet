...
// profilesV13 keeps the v10 run-size envelope for the evidence-bounded release.
// v13 changes the mix inside the fixed budget (point-in-time twins, grounded
// abstention pairs, realism-only staged corrections), not the scale; the
// envelope rebalance is a separate contract decision and must be pinned here
