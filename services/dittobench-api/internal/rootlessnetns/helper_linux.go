...
// requires fd 3 to be a SOCK_SEQPACKET Unix socket, creates one listening TCP
...
func RunHelper(args []string) int {
	// The namespace descriptors inherited through nsenter are no longer needed.
	// Close them before the Go runtime creates any descriptor of its own.
	if unix.CloseRange(helperSocketFD+1, math.MaxUint32, 0) != nil {
