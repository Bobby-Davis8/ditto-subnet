...
`src/catalog.rs` mirrors the **bench v13** production surface (regenerated by
`go run ./cmd/catalogmirror -format rust` in `research/dittobench-datagen`), so
it no longer advertises `set_main_model`. Until `ACTIVE_BENCH_VERSION` moves to
13, a live-bench dataset at bench 11/12 (`uv run ditto practice`) still emits
`set_model` cases that expect that tool; the kit cannot advertise or call it,
so those cases score 0 locally. That family does not exist at v13, where the
scored run advertises the surface in this file plus per-seed coined decoys.
