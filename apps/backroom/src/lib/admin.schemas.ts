...
export const sourceReviewInvariantAssessmentSchema = z
  ...
  .superRefine((assessment, context) => {
    ...
    const expected = sourceReviewInvariantSchema.options.filter(
      (invariant) => assessment.schema_version === 2 || invariant !== 'i8_evaluation_independence',
    )
    if (
      invariants.length !== expected.length
      || new Set(invariants).size !== expected.length
      || expected.some((invariant) => !invariants.includes(invariant))
    ) {
      context.addIssue({
        code: 'custom',
        message: assessment.schema_version === 2
          ? 'source review must decide every policy-v13 invariant'
          : 'source review must decide every policy-v10 invariant',
      })
    }
  })
