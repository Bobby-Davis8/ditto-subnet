...
export function createBackroomMcpServer(props: McpGrantProps) {
  ...
  registerTool(
    'get_score_history',
    {
      ...
      description:
        "One SN118 agent's accepted validator scores grouped per benchmark version, by agent UUID or miner hotkey, so version-over-version deltas come from the authoritative ledger instead of dashboard scraping. Each version group returns the accepted-score count, median/min/max composite, median tool and memory means, scoring window, validator hotkeys, seeds, and the median-composite delta against the previous version. A submission only carries rows for versions it was actually scored or re-scored on. Bench v13+ groups add `gate_posture` (the v13 gate posture when every gated row agrees, null when mixed or absent) and `median_gate_induced_loss` (median composite the gates would take in shadow, or did take in enforce, over the rows that carry evidence); both are null below v13. Seeds are exact decimal strings, not numbers, because a 63-bit seed does not fit a JavaScript number and a rounded seed reproduces a different dataset. Requires backroom:read and exposes no miner source.",
      inputSchema: agentScoresLookupInputSchema,
      annotations: toolAnnotations('read'),
    },
    async (input) =>
