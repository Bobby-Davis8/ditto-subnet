...
platform.env.j2 renders exactly these lines; this module proves the loader
accepts them. The seed below is the public synthetic test key, never production
configuration.
"""
...
def test_rendered_enabled_environment_names_the_fixed_seed_and_binds_the_key(
    ...
    assert config.seed_file == fixed
    assert fixed.parent == Path(
        defaults["platform_coding_hosted_signer_seed_directory"]
    )
...
def test_ansible_hotkey_guard_is_no_wider_than_the_platform_loader():
    guard = (ROLE / "tasks" / "coding_hosted_signer.yml").read_text()
    (pattern,) = re.findall(
        r"platform_coding_hosted_signer_hotkey is match\('([^']+)'\)", guard
    )
    assert pattern == "^5[1-9A-HJ-NP-Za-km-z]{47}$"
    ...
    check_hosted_signer_config(
        HostedControlSignerConfig(
            True,
            Path("/etc/ditto-platform/coding-hosted-signer/seed"),
            "5" + "1" * 47,
        )
    )
