...
class Top5ConfirmationJobRequest(BaseModel):
    """Fresh signed claim for the top-5 shared-seed rescore lane.
...
class JobResponse(BaseModel):
    ...
    benchmark_runtime: BenchmarkRuntimeSettings | None = Field(
        ...
        description=(
            ...
            "the scorer default of four overlapping /run calls with delay "
            "fingerprinting off. An explicit stamped 1 stays serial."
        ),
    )
...
class LedgerResponse(BaseModel):
    ...
    track_shares_bps: dict[str, int] = Field(
        ...
        description=(
            ...
            "resolved here and served already-decided like ``burn_share`` so "
            "the whole fleet folds one allocation. Empty (the default, and what "
            "an older platform's omission means) leaves the validator on its "
            "compiled ``TRACK_SHARES_BPS`` single-pool split, so a validator "
