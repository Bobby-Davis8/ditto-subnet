...
This role does not supply the missing private PostgreSQL VPC route, installed
custody services, production credentials, private release registration, native
images or canary approval. The fixed router and proxy endpoints for
`candidate_tcp` come from the separate
[host prerequisites role](coding-hosted-prerequisites-v2.md). Do not expose
